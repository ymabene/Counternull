#' Creates difference of means permutation vector for counternull distribution
#'
...
permutation_counter_diff_means<-function(sample_data, rand_matrix,
  ...
  for(k in 1:iterations)
  {
    ...
    # mean difference for exposed group
    on<-(sum(obs_on) + sum(est_on)) / (t + u - 2)
    # mean difference for non exposed
    off<-(sum(obs_off) + sum(est_off)) / (v + w - 2)
    counter_samples[k]<-(on-off)
